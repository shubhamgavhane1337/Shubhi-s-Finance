...
const FIREBASE_DOC_ID = 'main_data'; // Using a single document for simplicity
...
class Store {
    ...
    async loadData() {
        ...
        // Initial fallback load to display data instantly from localStorage
        if (stored) {
            try {
        ...
        if (fbConfigStr) {
            try {
                ...
                console.log("Firebase Initialized. Starting real-time sync...");
                
                // Set up real-time listener for the main document
                const docRef = doc(this.db, "finance_data", FIREBASE_DOC_ID);
                this.unsubscribe = onSnapshot(docRef, (docSnap) => {
                    if (docSnap.exists()) {
                        console.log("Firebase Real-time Update Received");
                        this.data = docSnap.data();
                        
                        // Ensure all default state fields exist to prevent errors if cloud state is old
                        this.data = { ...defaultState, ...this.data };
                        
                        // Save to localStorage as local cache
                        localStorage.setItem(STORAGE_KEY, JSON.stringify(this.data));
                        
                        this.recalculateBalances();
                        this.notifyListeners(); // Update UI automatically on all devices!
                    } else {
                        // Document doesn't exist yet on firebase, let's create it with current local data
                        console.log("Firebase document empty, initializing with local data...");
    ...
    saveData() {
        ...
        fetch('/api/data', {
            ...
            body: dataStr
        }).catch(err => {}); // Silent fail for localAPI if not running
    }
